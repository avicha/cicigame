...
Package = function (pac) {
    this.name = pac;
    this.path = config.packages_path + pac.replace('.', '/') + '/';
    this.modules = {};
    return this;
}
//定义包的创建模块方块
Package.prototype = {
    module:function (m) {
        if (this.modules[m]) {
            ...
            this.modules[m] = {isLoaded:false, owner:this.name, requires:[], body:null};
...
//居中画布
    centerScreen:function () {
        ...
        this.offsetX = Math.max(0, (this.viewport.width - parseFloat(this.canvas.style.width)) / 2);
        this.offsetY = Math.max(0, (this.viewport.height - parseFloat(this.canvas.style.height)) / 2);
